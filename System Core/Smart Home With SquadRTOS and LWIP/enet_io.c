...
#include "io.h"
#include "cgifuncs.h"
#include "kernal/inc/SquadRTOS.h"

//*****************************************************************************
//
//! <h1>Smart Home based in SquadRTOS and Ethernet </h1>
//!
//! <!--##### README BEGIN #####-->
...
static int flag = 1;
static char pcDecodedString[48];
//*****************************************************************************
//
...
UART0_IRQHandler(void)
{
    ...
    while(MAP_UARTCharsAvail(UART0_BASE))
    {
        ...
        char input = MAP_UARTCharGetNonBlocking(UART0_BASE);
        if(input == 'A'){
            flag = 0;
            SquadRTOS_SVC_vTaskResume(&thread2);
        }
...
void UARTprintf(char *pui8Buffer)
{
    //
    // Loop while there are more characters to send.
    //
    while(pui8Buffer[0] !='\0')
    {
        //
                  ...
                  char *pcValue[])
{
    ...
    //
    if(pcDecodedString[0] == '2')
    {
        flag = 1;
    }
    else
    {
       SquadRTOS_SVC_vTaskResume(&thread3);
    }
    //
...
// TCP/IP stack and SquadRTOS to control various peripherals on the board via a web
// browser.
//
//*****************************************************************************
...
void thread2function()
{
    while(1)
    {
        if(flag)
        {
            MAP_UARTCharPutNonBlocking(UART0_BASE, '2');
...
int main(void)
{
    ...
    //
    io_init();
    // Initialize SquadRTOS
    SquadRTOS_vinit();
    SquadRTOS_SVC_vthreadCreate(&thread1, &thread1stack, 2, thread1function);
    SquadRTOS_SVC_vthreadCreate(&thread2, &thread2stack, 1, thread2function);
    SquadRTOS_SVC_vthreadCreate(&thread3, &thread3stack, 2, thread3function);
